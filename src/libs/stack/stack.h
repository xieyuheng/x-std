...
typedef void (stack_destructor_t)(void **item);

stack_t *stack_new(size_t size);
void stack_purge(stack_t *self);
void stack_destroy(stack_t **self_pointer);

void stack_set_destructor(stack_t *self, stack_destructor_t *destructor);
stack_t *stack_new_with(size_t size, stack_destructor_t *destructor);
